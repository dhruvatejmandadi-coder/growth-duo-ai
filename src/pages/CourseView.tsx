...
export default function CourseView() {
  ...
  const switchModule = (index: number) => {
  ...
  return (
    <DashboardLayout>
      <div className="flex flex-col lg:flex-row h-[calc(100vh-3.5rem)]">
        {/* Sidebar - Module List */}
        <div className="w-full lg:w-72 border-b lg:border-b-0 lg:border-r border-border bg-card p-4 overflow-y-auto flex-shrink-0">
          <Button variant="ghost" size="sm" onClick={() => navigate("/courses")} className="mb-3 -ml-2 text-muted-foreground">
            <ArrowLeft className="w-4 h-4 mr-1" /> Back
          </Button>
          <h2 className="font-display font-bold text-lg mb-1 line-clamp-2">{course?.title}</h2>
            ...
            {modules.map((m, i) => (
              <button
                ...
                className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-left transition-colors ${
                  i === activeModule ? "bg-primary/10 text-primary" : "hover:bg-secondary"
                }`}
              >
                {m.completed ? (
            ...
            <div className="max-w-3xl mx-auto">
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => toggleComplete(mod.id, mod.completed)}
                >
                  {mod.completed ? (
              ...
              <div className="flex gap-1 mb-6 bg-secondary/50 p-1 rounded-lg w-fit">
                {[
                  ...
                  <button
                    key={key}
                    onClick={() => { setActiveTab(key as any); if (key !== "quiz") resetQuiz(); }}
                  ...
                  <div className="flex gap-3">
                    {!quizSubmitted ? (
                      <Button
                        ...
                        disabled={Object.keys(quizAnswers).length < (mod.quiz?.length || 0)}
                      >
                        Submit Quiz
                      </Button>
                    ) : (
              ...
              <div className="flex justify-between mt-8 pt-4 border-t border-border">
                <Button
                  ...
                  onClick={() => switchModule(activeModule - 1)}
                >
